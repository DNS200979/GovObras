    ...
    <div className="flex min-h-screen items-center justify-center bg-concreto p-6">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-lg border border-linha bg-papel p-7 shadow-sm">
          ...
          {erro === "papel" ? (
            <p className="mb-4 rounded-sm border border-ambar/40 bg-ambar/10 px-3 py-2 text-[13px] text-ambar">
              Esta conta não é de concreteira. Contas da construtora acessam o CarbonFree Obra.
            </p>
          ) : null}
          {/* direto para a rota final: encadear com o redirect de "/" quebra o Server Action de login */}
          <LoginForm next={next ?? "/obras"} />
        </div>

        <p className="mt-6 text-center font-mono text-[10.5px] tracking-wide text-texto-fraco">
          MBV · Movimento Brasil Verde · Concreteiras
        </p>
      </div>
    </div>
  );
